...
function NewsDetailWindow(id) {
	//declare module dependencies
	
	var NewsDetailMenuView = require('ui/common/News/NewsMenuBarView');
	//construct UI
	var newsDetailMenuView = new NewsDetailMenuView();
	var newsId = id;
	var db = Ti.Database.open('SqlPaoApp');
	var rows = db.execute('SELECT NIDFK, PictureUrl FROM NewsPictures where NIDFK =' + newsId);
	...
	while(rows.isValidRow())
	{
		if(left == 10)
		{
		//	totalView.setImage("http://apishop.planatoffice.nl/" + rows.fieldByName('PictureUrl'));
		}
	...
	var rows2 = db.execute('SELECT NID, Title, OverviewPicture, Description FROM News where NID = ' + id);
	
	while (rows2.isValidRow()){
	  titleArticle.setText(rows2.fieldByName('Title'));
	  descriptionLabel.setText(rows2.fieldByName('Description'));
	  rows2.next();
	}
	...
	totalView.add(newsDetailMenuView);
...
module.exports = NewsDetailWindow;