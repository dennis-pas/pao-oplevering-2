...
//Rare exceptions to indicate a bug in the _paoapp2 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_paoapp2 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_paoapp2]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.paoapp2.module"
